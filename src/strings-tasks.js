...
function removeFirstOccurrences(str, value) {
  const prev = str.indexOf(value);
  const len = value.length;

  return str.includes(value)
    ? str.slice(0, prev).concat(str.slice(prev + len))
    : str;
}
...
function removeLastOccurrences(str, value) {
  const prev = str.lastIndexOf(value);
  const len = value.length;
...
function reverseWords(str) {
  ...
  return result.map((i) => i.split('').reverse().join('')).join(' ');
}
...
function extractNameFromTemplate(value) {
  return value.split('!')[0].split(',')[1].trimStart();
}
...
function encodeToRot13(str) {
  ...
  const part_1 = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const part_2 = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  return str.replace(/[a-z]/gi, (letter) => part_2[part_1.indexOf(letter)]);
}
